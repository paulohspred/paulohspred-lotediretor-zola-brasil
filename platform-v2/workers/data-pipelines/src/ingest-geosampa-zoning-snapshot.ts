...
const CONNECTOR_VERSION = 'geosampa-zoning-wfs-v1';
const TYPE_NAME = 'geoportal:perimetro_zona_lei_18177_24';
...
function parseOptions(): CliOptions {
  ...
  return {
    sourceCode: readArg('source-code') ?? 'PMSP_GEOSAMPA_ZONEAMENTO',
...
async function resolveSource(pool: Pool, options: CliOptions): Promise<SourceRow> {
  ...
  if (source.method !== 'GET') throw new Error(`GeoSampa zoning connector requires GET endpoint, found ${source.method}`);
  return source;
}
...
function buildRequestUrl(source: SourceRow, bbox: string): string {
...
function featureCount(payload: Record<string, unknown>): number {
  ...
  if (!Array.isArray(features)) throw new Error('GeoSampa zoning response has no features array');
  if (features.length > 50) throw new Error(`GeoSampa zoning response exceeded candidate limit: ${features.length}`);
  return features.length;
}

async function downloadZoning(
  source: SourceRow,
  options: CliOptions,
  lotGeometry: LotGeometryRef,
): Promise<DownloadedZoning> {
  const requestUrl = buildRequestUrl(source, lotGeometry.bbox);
  ...
  try {
    const response = await fetch(requestUrl, {
      ...
      headers: {
        ...
        'user-agent': 'LoteDiretor-GeoSampaZoningSnapshot/0.1 (+https://github.com/paulohspred/paulohspred-lotediretor-zola-brasil)',
      },
    });
...
async function main(): Promise<void> {
  ...
  const pool = new Pool({
    ...
    application_name: 'lotediretor-geosampa-zoning-ingest',
    max: 2,
  });
  ...
  try {
    ...
    const downloaded = await downloadZoning(source, options, lotGeometry);
