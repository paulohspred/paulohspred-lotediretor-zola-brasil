...
const pollMs = Math.max(500, Number(process.env.MATERIALIZATION_POLL_MS ?? 2000));
const staleAfterMinutes = Math.max(
  5,
  Number(process.env.MATERIALIZATION_STALE_MINUTES ?? 15),
);
...
function textField(result: JsonResult, field: string): string {
  const value = result[field];
  if (typeof value !== 'string' || !value) {
    throw new Error(`Pipeline result is missing ${field}`);
  }
  return value;
}
...
async function claimJob(pool: Pool): Promise<JobRow | null> {
  ...
  try {
    ...
    await client.query(
      `UPDATE core.property_materialization_job
       SET status = 'RUNNING',
...
async function materialize(job: JobRow): Promise<JsonResult> {
  if (job.subject_type !== 'SP_LOT') {
    throw new Error(`Unsupported subject type: ${job.subject_type}`);
  }
  ...
  const lotSnapshot = await runJsonScript(
    'ingest-geosampa-lot-snapshot.js',
    baseArgs,
  );
  await runJsonScript('parse-geosampa-lot-snapshot.js', [
    '--snapshot-id',
    textField(lotSnapshot, 'snapshotId'),
  ]);

  const zoningSnapshot = await runJsonScript(
    'ingest-geosampa-zoning-snapshot.js',
    baseArgs,
  );
  const zoningEvidence = await runJsonScript(
    'parse-geosampa-zoning-snapshot.js',
    ['--snapshot-id', textField(zoningSnapshot, 'snapshotId')],
  );

  const territorial: Record<string, JsonResult> = {};
  for (const layerKey of ['macrozona', 'macroarea', 'risco_hidrologico']) {
    const snapshot = await runJsonScript(
      'ingest-geosampa-territorial-snapshot.js',
      [...baseArgs, '--layer-key', layerKey],
    );
    territorial[layerKey] = await runJsonScript(
      'parse-geosampa-territorial-snapshot.js',
      ['--snapshot-id', textField(snapshot, 'snapshotId')],
    );
  }

  return {
    lotId: job.subject_id,
    lotSnapshotId: textField(lotSnapshot, 'snapshotId'),
    zoningSnapshotId: textField(zoningSnapshot, 'snapshotId'),
    zoningEvidenceCount: zoningEvidence.evidenceCount ?? 0,
    territorialEvidenceCounts: Object.fromEntries(
      Object.entries(territorial).map(([key, value]) => [
        key,
...
async function main(): Promise<void> {
  const pool = new Pool({
    ...
    application_name: 'lotediretor-materialization-worker',
    max: 3,
  });
  ...
  try {
    ...
    console.log(
      JSON.stringify({
        event: 'materialization-worker-started',
        workerId,
        pollMs,
      }),
    );
    ...
    while (!stopping) {
      ...
      try {
        ...
        console.log(
          JSON.stringify({
            event: 'materialization-succeeded',
            jobId: job.id,
            subjectId: job.subject_id,
            result,
          }),
