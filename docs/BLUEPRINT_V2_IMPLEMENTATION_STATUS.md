...
- Evidence API read-only + primeiro parser factual HTML (`SOURCE_DOCUMENT_TITLE`) com verificação do SHA do snapshot, citação e idempotência; primeira evidência oficial LPUOS comprovada em desenvolvimento. Evidências espaciais calculadas recebem assessment automático de integridade/proveniência (`EXACT_2D_OVERLAP`) sem atribuir confiança semântica. Faltam ampliar assessments por domínio e modelar conflitos explicitamente.
